...
void LDB_Deinit(LDB_Instance *inst)
{
	if (!inst) return;
	...
	if (inst->options)
	{
		LDB_OptionsDestroy(inst->options);
		inst->options = NULL;
	}
	if (inst->roptions)
	{
		LDB_ReadOptionsDestroy(inst->roptions);
		inst->roptions = NULL;
	}
	if (inst->woptions)
	{
		LDB_WriteOptionsDestroy(inst->woptions);
		inst->woptions = NULL;
	}
	if (inst->errors)
	{
		LDB_Free(inst->errors);
		inst->errors = NULL;
	}
}
