...
static inline u64 compact_size_decode(u8 **pp, const u8 *end)
{
    ...
    else if (first == 0xFE)
    {
        ...
        u64 val = (u64)p[0] | ((u64)p[1] << 8) | ((u64)p[2] << 16) |
                  ((u64)p[3] << 24);
        *pp = p + 4;
        return val;
    }
...
static inline int encode_varint_msb(u32 n, u8 *out)
{
    int len = 0;
    u8 tmp[10]; // Fixed maximum buffer size to satisfy compiler analysis
    ...
    while (1)
    {
        ...
        // Explicit bound tells GCC's -O3 vectorizer this loop is small,
        // preventing the 32-byte stringop-overflow warning.
        if (len >= 9)
            break;
    }
